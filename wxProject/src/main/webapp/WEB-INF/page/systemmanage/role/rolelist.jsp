...
<div class="page" id="panel_${uuid}">
	<div>
		...
		<div class="kuangxian">
			<table class="table1">
				<tr>
					<th style="width:1%;">角色名称：<input type="text" class="text1" id="name_${uuid}"><input class="button" type="button" value="查询" onclick="qryRoleList();"/></th>
<!-- 					<td></td> -->

<!-- 					<td><input class="button" type="button" value="查询" onclick="qryRoleList();"/></td> -->
				</tr>
			</table>
		</div>
	</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			//工具栏操作按钮
			var toolbar = [];
			<shirox:hasAnyPermissions permissionKeys="rolemanage:add">
				toolbar.push({
					text:'新增',
					iconCls:'icon-add',
			...
			<shirox:hasAnyPermissions permissionKeys="rolemanage:edit">
				toolbar.push({
					text:'修改',
					iconCls:'icon-edit',
			...
			<shirox:hasAnyPermissions permissionKeys="rolemanage:delete">
				toolbar.push({
					text:'删除',
					iconCls:'icon-remove',
			...
			function operCol(method){
				...
				if(method=='add'){
					$.jumpPage2('${uuid}',"${ctx}/systemmanage/role/initDetail",{oper:'add'});
				}else if(method=='view'){
					$.jumpPage2('${uuid}',"${ctx}/systemmanage/role/initDetail",{oper:'view',id:id});
				}else if(method=='edit'){
					$.jumpPage2('${uuid}',"${ctx}/systemmanage/role/initDetail",{oper:'edit',id:id});
					...
					$.messager.confirm("提示信息","你确定要删除数据吗？",function (isConfirm){
						if(!isConfirm) return;
						$.ajax({
							url:"${ctx}/systemmanage/role/delete?id="+id,
							type : "POST",
							dataType : "json",
			...
			$('#tb_${uuid}').datagrid({
			    ...
			    url:'${ctx}/systemmanage/role/search',
			    idField:'id',
			    toolbar:toolbar,
			    columns:[[
					{title:'角色名称',field:'name',width:100,align:'center',halign:'center'},
					{title:'描述',field:'description',width:300,align:'center',halign:'center'}
			    ]],
